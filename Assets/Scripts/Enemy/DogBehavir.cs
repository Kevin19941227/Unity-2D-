...
public class DogBehavior : Enemy
{
    [Header("Movement")]
    ...
    void Update()
    {
        ...
        if (currentDirection.x > 0) transform.localScale = new Vector3(10, 10, 1);
        else if (currentDirection.x < 0) transform.localScale = new Vector3(-10, 10, 1);
    }
    ...
    private void Die()
    {
        if (deathSound != null)
        {
            GameObject deathAudioObj = new GameObject("DeathSound");
            AudioSource tempSource = deathAudioObj.AddComponent<AudioSource>();
